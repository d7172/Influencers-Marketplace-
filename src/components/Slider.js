    ...
    <div className="w-[1128px] relative">
      {activeSlide !== 0 && (
        <button className="bg-white shadow absolute left-0 top-16 rounded-full p-4 ">
          <ChevronLeftIcon className="w-5 h-5 text-[#3751FF]" aria-hidden="true" onClick={sliderRef?.slickPrev} />
        </button>
      )}
      <div className="px-14">
        <Slider ref={setSliderRef} {...sliderSettings}>
          {detailsCard.map((card, id) => (
            <div key={id}>
              <div
                className={`w-[245px] cursor-pointer h-[184px] top-[393px] left-[440px] flex flex-col items-center justify-center border-solid border-2 border-[${
                  card.status === 1 ? active : inactive
                }]`}
              >
                {card.status === 1 && (
                  <p className="text-[#3751FF]">
                    <span className="bg-[#3751FF] rounded-full inline-block mr-2 w-3 h-3"></span>On going
                  </p>
                )}
                <h1 className="text-[18px] font-[500] text-[#000000] my-2">Campaign : {card.id}</h1>
                <div
                  className={`text-[${card.status === 1 ? active : inactive}] border-[${
                    card.status === 1 ? active : inactive
                  }] font-bold border-2 border-dashed w-[165px] h-[42px] top-[473px] left-[485px] flex items-center justify-center`}
                >
                  <h1>&#8377;{card.amount}</h1>
                </div>
                <h1 className={`text-[${card.status === 1 ? active : inactive}] underline mt-4 cursor-pointer`}>
                  Click here
                </h1>
              </div>
            </div>
      ...
      {activeSlide !== totalActiveSlide && (
        <button className="bg-white absolute right-0 shadow top-16 rounded-full p-4 ">
          <ChevronRightIcon className="w-5 h-5 text-[#3751FF]" aria-hidden="true" onClick={sliderRef?.slickNext} />
        </button>
      )}
      {/* <button className="bg-white absolute right-0 shadow top-16 rounded-full p-4 " >
