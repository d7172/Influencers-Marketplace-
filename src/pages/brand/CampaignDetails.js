...
function CampaignDetails({ route }) {
  ...
  const campaignDetails = useSelector((state) =>
    isAssigned
      ? state.BrandAssignedCampaign?.results?.filter((i) => i?.campaigndetails?.id == id)[0]
      : isActive
      ? state?.BrandActiveCampaign?.results?.filter((i) => i.id == id)[0]
      ...
      <div className="w-full bg-[#F2F2F2] py-4 px-8">
        <Breadcrumbs
          options={[
            { title: "Campaign" },
            {
              title: isAssigned ? "Assigned Campaign" : isActive ? "Active Campaign" : "Rejected Campaign",
              onClick: () => {
      ...
      <div className="mt-6 px-6 pb-10">
        ...
        >
          <ResonForRejction
            ...
            onSubmit={() => {
              isAssigned && updateQuotion("reject", reason);
              setRejectionDialog(false) || (isActive && updateQuotion("reject", reason));
            }}
            reason={reason}
            setReason={setReason}
          />
        ...
        <div className="ml-4">
          ...
          <div className="flex w-full justify-between">
            <div className="mt-6">
              ...
              <p className="text-[18px] font-[500] text-[#969BA0] ">
                {" "}
                {campaignDetails?.id || campaignDetails?.campaigndetails?.id}{" "}
              </p>
            </div>
            {isRejected && (
              <div>
                ...
                {campaignDetails?.quotationdetails?.map((data, i) => {
                  return (
                    <div
                      ...
                      key={i}
                      onClick={() => setActiveQuotation(data?.id)}
                    >
                      <p className="font-bold">Quotion</p>
                      <p className="font-bold">{i + 1}</p>
                      <button
                        className={`px-6 my-4 py-2 border-2 border-dashed ${
                          activeQuotation === data?.id
                            ? `border-[#3751FF] text-[#3751FF]`
                            : `border-[#969BA0] text-[#969BA0]`
                        } `}
                      >
                        &#8377;{data?.bid_amount}
                      </button>
          ...
          {(isActive || isAssigned) && (
            <div className="flex w-[50%] justify-end">
              <div>
                <button
                  onClick={() => {
                    (isAssigned && updateQuotion("active", "")) || (isActive && updateQuotion("active", ""));
                  }}
          ...
          {isActive && (
            <>
              <div className="flex p items-start my-8">
                ...
                <div>
                  <h4 className="font-[600] text-[18px]">Hold Payment: &#8377;4840</h4>
