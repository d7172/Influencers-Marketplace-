...
function BrandCampaignDeliverables({
  mainRoute,
  setDocReqDialog,
  route,
  setPaymentDialog,
  setDocumentPhaseDialog,
  deliverableDetails,
}) {
  let bidTotal = 0;
  return (
    <div>
              ...
              <thead>
                <tr className="flex w-auto mb-4">
                  {route === "admin/active-campaign" && (
                    <th scope="col" className="text-left text-[#6C6C6C] font-[500] text-lg w-[137px] mr-4">
                      Influencer name
                    </th>
                  )}
                  <th scope="col" className="text-left text-[#6C6C6C] font-[500] text-lg w-[137px] mr-4">
                  ...
                  <th scope="col" className="text-left text-[#6C6C6C] font-[500] text-lg w-[155px] mr-4">
                    Deliverables
                  </th>
                  <th scope="col" className="text-left text-[#6C6C6C] font-[500] text-lg w-[80px] mr-4">
                    Duration
                  </th>
                  <th scope="col" className="text-left text-[#6C6C6C] font-[500] text-lg mr-4">
                    Engagement rate
                  </th>
                  <th scope="col" className="text-left text-[#6C6C6C] font-[500] text-lg w-[75px] mr-4">
                    Amount
                  </th>
                </tr>
              </thead>
                ...
                {deliverableDetails?.map((data, index) => {
                  ...
                  return (
                    <tr key={index} className="flex mb-8">
                      {route === "admin/active-campaign" && (
                        <td className="min-w-[137px] mr-4">
                          <p>{data?.infName}</p>
                        </td>
                      )}
                      <td className="flex items-start w-[137px] mr-4 capitalize">
                        <img src={`/svgs/${data?.platform}.svg`} className="w-[20px] h-[20px] mr-2" />
                        <p className="capitalize">{data?.platform}</p>
                      </td>
                      <td className="flex flex-col gap-4 w-[155px] mr-4">
                      ...
                      {route === "brand/active-campaign" && (
                        <td className="flex flex-col gap-4 w-[75px] mr-4 ">
                          {data.documentsLinks?.map((data) => {
                            return <a className="underline text-[#3751FF] cursor-pointer">Click here</a>;
                          })}
                        </td>
                      )}
                      {route === "brand/active-campaign" && (
                        <a
                          className="underline text-[#3751FF] cursor-pointer"
                ...
                <div className="w-[505px]">
                  ...
                  <div className="my-16 flex w-full justify-end gap-4">
                    <h1 className="text-2xl font-bold mr-4">
                      {route.includes("admin") ? `Influencers Bid Total` : `Portal Quotation`}
                    </h1>
                    <p className="font-bold text-2xl text-[#3751FF]">&#8377;{bidTotal}</p>
                  </div>
                  <div className="w-72 ml-auto h-1  bg-[#EEEEEE]"></div>
                  <div className="my-16 flex w-full justify-end gap-4">
                    <h1 className="text-2xl font-bold mr-4 w-[235px]">
                      {route.includes("admin") ? `Your Bid` : `Your Amount`}
                    </h1>
                    <p className="font-bold text-2xl text-[#3751FF]">&#8377;6000</p>
                  </div>
                  <div className="w-[578px] my-8 h-[2px] bg-[#EEEEEE]"></div>
                  {route === "admin/assigned-campaign" && (
                    <div className="my-16 items-end flex flex-col text-right w-full justify-end gap-4">
                      <h1 className="text-2xl font-bold ml-8 w-[235px]">Your margin</h1>
                      <input
                        type="text"
                        className="w-[177px] h-12 px-4 text-[#3751FF] border rounded-sm text border-gray-400 focus:outline-none"
                      />
                    </div>
                  )}
                </div>
      ...
      {route === "admin/assigned-campaign" && (
        <div className="mt-14 flex w-[50%] justify-end">
          <div>
            <button
        ...
        <div>
          <div className="flex p items-start my-8">
            <div className="flex flex-col">
              <h4 className="font-[600]">Payment Sent Details by Brand</h4>
              <table>
                <thead>
            ...
            <div>
              <h4 className="font-[600]">Hold Payment: &#8377;4840</h4>
          ...
          <div className="w-[50%] mt-10">
            <button
              ...
              className="bg-[#3751FF] px-8 py-4 rounded-full text-white mr-10"
            >
