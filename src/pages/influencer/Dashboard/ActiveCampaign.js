...
function ActiveCampaign() {
  ...
  activeCampaignState = useSelector((state) => state?.infCampaignActive?.results);
...
function ActiveCampaignTable() {
  ...
  return (
    <div className="mt-6 pr-4">
      ...
      {activeCampaignState?.map((activeCampaign) => {
        return (
          <div key={activeCampaign?.id}>
            <div className="flex gap-10 px-2 py-4 text-sm text-gray-900 whitespace-nowrap items-start">
              <div className=" w-[95px] font-medium">{activeCampaign?.id}</div>
              ...
              <div className="w-[80px] ml-1.5">&#8377; {activeCampaign?.amount}</div>
