...
function BidDetails() {
  ...
  console.log(AdmActiveBidDetails);
  return (
    <>
      ...
      <div className="ml-10">
        <div className="px-8 py-5">
          ...
          <p className="w-390 inline-block text-gray-500 text-sm text-start m-auto mb-4">#{AdmActiveBidDetails.campaign_details.id}</p>
          ...
          {AdmActiveBidDetails.bid_details.map((data) => (
            <div className="flex mt-8 gap-20" key={data.id}>
              <div
                className={`w-[258px] h-[184px] top-[393px] left-[440px] flex flex-col items-center justify-center cursor-pointer pr-4 border-solid border-2 border-[${bidTab === data.id ? active : inactive}]`} onClick={() => setBidTab(data.id)}
              >
                <h1 className="text-[18px] font-[500] text-[#000000] my-2">Bid Number :{data.id}</h1>
                <div
                  className={`text-[${bidTab === data.id ? active : inactive}] border-[${bidTab === data.id ? active : inactive}] font-bold border-2 border-dashed w-[165px] h-[42px] top-[473px] left-[485px] flex items-center justify-center`}
                >
                  <h1>&#8377;{data.influencer_bid_amount}</h1>
                </div>
                <h1 className={`text-[${bidTab === data.id ? active : inactive}] underline mt-4 cursor-pointer`}>Click here</h1>
              </div>
            </div>
          ))}
          <div className="mt-8">
